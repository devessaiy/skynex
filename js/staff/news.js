...
(async () => {
  ...
  function renderList() {
    ...
    listEl.innerHTML = rows.map(p => {
      ...
      return /*html*/`
      <div class="bg-white border border-skynex-border rounded-2xl p-5 flex items-start justify-between gap-4">
        <div class="min-w-0">
          <div class="flex items-center gap-2 mb-1 flex-wrap">
            <p class="font-bold text-skynex-dark truncate">${Staff.esc(p.title)}</p>
            <span class="inline-block px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${left <= 1 ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'}">${left === 0 ? 'expires today' : `${left}d left`}</span>
          </div>
          <p class="text-sm text-slate-500 truncate">${Staff.esc(p.author_name)} · published ${Staff.esc(Staff.formatDate(p.published_at))}</p>
        </div>
        <div class="shrink-0 flex gap-2">
          <button data-edit="${p.id}" class="w-9 h-9 rounded-full border border-skynex-border flex items-center justify-center text-slate-500 hover:text-skynex-dark hover:border-skynex-dark transition-colors" aria-label="Edit"><i class="fa-solid fa-pen text-xs"></i></button>
          <button data-delete="${p.id}" class="w-9 h-9 rounded-full border border-skynex-border flex items-center justify-center text-slate-500 hover:text-red-600 hover:border-red-600 transition-colors" aria-label="Delete"><i class="fa-solid fa-trash text-xs"></i></button>
        </div>
      </div>`;
    }).join('');
    ...
    const { body, close } = Staff.drawer({ title: isEdit ? 'Edit Post' : 'New Post', html: /*html*/`
      <form data-form class="space-y-5">
        ...
        <div>
          ...
          ${existing?.image_path ? `<img src="${Staff.esc(Media.publicUrl('news-media', existing.image_path))}" class="w-full aspect-video object-cover rounded-lg mb-2 bg-skynex-gray">` : ''}
          <input data-image type="file" accept="image/png,image/jpeg,image/webp" class="text-sm">
          ${existing?.image_path ? `<label class="flex items-center gap-2 text-xs text-slate-500 mt-2"><input type="checkbox" data-remove-image> Remove current image</label>` : ''}
        </div>
        <div>
          <label class="${Staff.labelCls}">Video <span class="normal-case font-normal text-slate-400">(optional, MP4/WebM, 50MB max)</span></label>
          ${existing?.video_path ? `<p class="text-xs text-slate-500 mb-2"><i class="fa-solid fa-circle-play"></i> Current video attached</p>` : ''}
          <input data-video type="file" accept="video/mp4,video/webm" class="text-sm">
          ${existing?.video_path ? `<label class="flex items-center gap-2 text-xs text-slate-500 mt-2"><input type="checkbox" data-remove-video> Remove current video</label>` : ''}
        </div>
