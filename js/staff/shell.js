...
const Staff = {
  ...
  logoHtml: (cls = 'h-6 w-auto') => /*html*/`
    <img src="/skynex-online.svg" alt="Skynex" class="${cls}"
      onerror="this.replaceWith(Object.assign(document.createElement('span'),{textContent:'Skynex',className:'text-lg font-bold tracking-tighter text-skynex-dark'}))">`,
  ...
  fatal(root, title, body) {
    root.className = 'min-h-screen flex items-center justify-center px-6 bg-skynex-gray';
    root.innerHTML = /*html*/`
      <div class="text-center max-w-md">
        <h1 class="text-2xl font-bold text-skynex-dark tracking-tighter mb-4">${title}</h1>
        <p class="text-slate-500 font-light mb-8">${body}</p>
        <button data-logout class="text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-skynex-dark transition-colors">Log Out</button>
      </div>`;
  ...
  renderFrame(root, { active, perms, profile, roleName, displayName }) {
    const links = Staff.NAV.filter(i => !i.perm || perms.has(i.perm)).map(i => /*html*/`
      <a href="${i.href}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-colors ${i.key === active ? 'bg-skynex-gray text-skynex-dark font-semibold' : 'text-slate-500 hover:bg-skynex-gray hover:text-skynex-dark font-medium'}">
        <i class="fa-solid ${i.icon} w-5 text-center"></i>${i.label}
      </a>`).join('');

    root.className = '';
    root.innerHTML = /*html*/`
      <div class="min-h-screen lg:flex bg-skynex-gray">
        <div id="staff-backdrop" class="hidden fixed inset-0 z-30 bg-skynex-dark/50 lg:hidden"></div>

        <aside id="staff-sidebar" class="fixed inset-y-0 left-0 z-40 w-72 max-w-[85vw] h-screen shrink-0 lg:sticky lg:top-0 bg-white border-r border-skynex-border flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-300">
          <div class="h-20 px-6 flex items-center gap-3 border-b border-skynex-border shrink-0">
            ${Staff.logoHtml('h-6 w-auto')}
            <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400 border-l border-skynex-border pl-3">Staff Portal</span>
          </div>
          <nav class="flex-1 overflow-y-auto p-4 space-y-1">${links}</nav>
          <div class="p-4 border-t border-skynex-border shrink-0 space-y-3">
            <div class="flex items-center gap-3 px-2">
              ...
              <div class="min-w-0">
                <p class="text-sm font-semibold text-skynex-dark truncate">${Staff.esc(displayName)}</p>
                <p class="text-xs text-slate-500 truncate">${Staff.esc(roleName)}${profile.position_title ? ' · ' + Staff.esc(profile.position_title) : ''}</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <a href="/" class="text-center text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-skynex-dark border border-skynex-border rounded-full py-2.5 transition-colors">Website</a>
              <button data-logout class="text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-white hover:bg-skynex-dark border border-skynex-border hover:border-skynex-dark rounded-full py-2.5 transition-colors">Log out</button>
            </div>
          </div>
        </aside>

        <div class="flex-1 min-w-0">
          <header class="lg:hidden sticky top-0 z-20 h-16 px-4 flex items-center justify-between bg-white/90 backdrop-blur-xl border-b border-skynex-border">
            <button id="staff-menu-btn" aria-label="Open menu" class="p-2 text-skynex-dark"><i class="fa-solid fa-bars text-xl"></i></button>
  ...
  pageHeader: (title, subtitle = '', actionHtml = '') => /*html*/`
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
      <div>
        <h1 class="text-3xl md:text-4xl font-bold text-skynex-dark tracking-tighter">${title}</h1>
        ${subtitle ? `<p class="text-slate-500 font-light mt-2 max-w-2xl">${subtitle}</p>` : ''}
      </div>
      ${actionHtml}
    </div>`,
  ...
  ghostBtn: 'inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full border border-skynex-border text-skynex-dark text-xs font-bold tracking-widest uppercase hover:border-skynex-dark transition-colors disabled:opacity-50',
  inputCls: 'w-full border border-skynex-border rounded-lg px-4 py-3 text-base text-skynex-dark bg-white focus:outline-none focus:border-skynex-dark transition-colors',
  labelCls: 'block text-xs font-bold uppercase tracking-widest text-slate-500 mb-2',
  ...
  confirm({ title, message, confirmLabel = 'Confirm', danger = false }) {
    return new Promise(resolve => {
      ...
      wrap.innerHTML = /*html*/`
        <div class="w-full max-w-sm bg-white rounded-2xl p-8" role="dialog" aria-modal="true">
          <h2 class="text-lg font-bold text-skynex-dark tracking-tighter mb-2">${Staff.esc(title)}</h2>
          <p class="text-sm text-slate-500 font-light mb-8">${Staff.esc(message)}</p>
  ...
  drawer({ title, html = '', onClose = null }) {
    ...
    wrap.innerHTML = /*html*/`
      ...
      <div class="absolute inset-y-0 right-0 w-full sm:max-w-xl bg-white shadow-2xl flex flex-col" role="dialog" aria-modal="true">
        <div class="h-20 px-6 sm:px-8 flex items-center justify-between border-b border-skynex-border shrink-0">
          <h2 class="text-lg font-bold text-skynex-dark tracking-tighter truncate">${Staff.esc(title)}</h2>
          <button data-close aria-label="Close" class="p-2 text-slate-400 hover:text-skynex-dark transition-colors"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
  ...
  statusBadge(status) {
    const map = {
      new: 'bg-blue-50 text-skynex-blue', reviewed: 'bg-amber-50 text-amber-700', resolved: 'bg-green-50 text-green-700',
      live: 'bg-green-50 text-green-700', scheduled: 'bg-amber-50 text-amber-700', visible: 'bg-green-50 text-green-700', hidden: 'bg-slate-100 text-slate-500'
    };
    return `<span class="inline-block px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${map[status] || 'bg-slate-100 text-slate-500'}">${Staff.esc(status)}</span>`;
  }
};
