...
const Staff = {
  ...
  fatal(root, title, body) {
    ...
    root.innerHTML = /*html*/`
      <div class="text-center max-w-md">
        ...
        <p class="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-light mb-8">${body}</p>
        <button data-logout class="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-skynex-dark dark:text-white transition-colors">Log Out</button>
      </div>`;
  ...
  renderFrame(root, { active, perms, profile, roleName, displayName }) {
    const links = Staff.NAV.filter(i => !i.perm || perms.has(i.perm)).map(i => /*html*/`
      <a href="${i.href}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-colors ${i.key === active ? 'bg-skynex-gray dark:bg-neutral-950 text-skynex-dark dark:text-white font-semibold' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:bg-skynex-gray dark:bg-neutral-950 hover:text-skynex-dark dark:text-white font-medium'}">
      ...
      <div class="min-h-screen lg:flex bg-skynex-gray dark:bg-neutral-950">
        ...
        <aside id="staff-sidebar" class="fixed inset-y-0 left-0 z-40 w-72 max-w-[85vw] shrink-0 lg:sticky lg:top-0 lg:h-screen bg-white dark:bg-neutral-900 border-r border-skynex-border dark:border-neutral-800 flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-300">
          <div class="h-20 px-6 flex items-center gap-3 border-b border-skynex-border dark:border-neutral-800 shrink-0">
            ...
            <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 border-l border-skynex-border dark:border-neutral-800 pl-3">Staff Portal</span>
          </div>
          ...
          <div class="p-4 pb-[max(1rem,env(safe-area-inset-bottom))] border-t border-skynex-border dark:border-neutral-800 shrink-0 space-y-3">
            <div class="flex items-center gap-3 px-2">
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 truncate">${Staff.esc(roleName)}${profile.position_title ? ' · ' + Staff.esc(profile.position_title) : ''}</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <a href="/" class="text-center text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-skynex-dark dark:text-white border border-skynex-border dark:border-neutral-800 rounded-full py-2.5 transition-colors">Website</a>
              <button data-logout class="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-white hover:bg-skynex-dark border border-skynex-border dark:border-neutral-800 hover:border-skynex-dark rounded-full py-2.5 transition-colors">Log out</button>
            </div>
          </div>
        </aside>

        <div class="flex-1 min-w-0">
          <header class="lg:hidden sticky top-0 z-20 h-16 px-4 flex items-center justify-between bg-white/90 dark:bg-neutral-950/90 backdrop-blur-xl border-b border-skynex-border dark:border-neutral-800">
  ...
  pageHeader: (title, subtitle = '', actionHtml = '') => /*html*/`
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
      <div>
        ...
        ${subtitle ? `<p class="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-light mt-2 max-w-2xl">${subtitle}</p>` : ''}
      </div>
      ${actionHtml}
    </div>`,
  ...
  labelCls: 'block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-2',
    ...
    return new Promise(resolve => {
      ...
      wrap.innerHTML = /*html*/`
        <div class="w-full max-w-sm bg-white dark:bg-neutral-900 rounded-2xl p-8" role="dialog" aria-modal="true">
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 font-light mb-8">${Staff.esc(message)}</p>
    ...
    wrap.innerHTML = /*html*/`
      ...
      <div class="absolute inset-y-0 right-0 w-full sm:max-w-xl bg-white dark:bg-neutral-900 shadow-2xl flex flex-col" role="dialog" aria-modal="true">
        <div class="h-20 px-6 sm:px-8 flex items-center justify-between border-b border-skynex-border dark:border-neutral-800 shrink-0">
          ...
          <button data-close aria-label="Close" class="p-2 text-slate-400 dark:text-slate-500 hover:text-skynex-dark dark:text-white transition-colors"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
  ...
  statusBadge(status) {
    const map = {
      ...
      live: 'bg-green-50 dark:bg-green-950/30 text-green-700', scheduled: 'bg-amber-50 dark:bg-amber-950/30 text-amber-700', visible: 'bg-green-50 dark:bg-green-950/30 text-green-700', hidden: 'bg-slate-100 dark:bg-neutral-800 text-slate-500 dark:text-slate-400 dark:text-slate-500'
    };
    return `<span class="inline-block px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${map[status] || 'bg-slate-100 dark:bg-neutral-800 text-slate-500 dark:text-slate-400 dark:text-slate-500'}">${Staff.esc(status)}</span>`;
  }
};
