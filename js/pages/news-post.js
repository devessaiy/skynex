...
(async () => {
  ...
  const gone = (text) => {
    box.innerHTML = /*html*/`
      <div class="text-center py-16">
        <h1 class="text-3xl md:text-4xl font-bold text-skynex-dark tracking-tighter mb-4">${text}</h1>
        <a href="/news" class="text-sm font-bold uppercase tracking-widest text-skynex-blue hover:text-skynex-dark transition-colors">Back to news</a>
      </div>`;
  };
  ...
  try {
    ...
    const paragraphs = p.body.split(/\n{2,}/).map(t => t.trim()).filter(Boolean)
      .map(t => `<p class="text-lg text-slate-600 font-light leading-relaxed">${UI.esc(t).replace(/\n/g, '<br>')}</p>`).join('');

    box.innerHTML = /*html*/`
      <a href="/news" class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-skynex-dark transition-colors mb-10">
        <i class="fa-solid fa-arrow-left text-[10px]"></i> All news
      </a>
      <h1 class="text-4xl md:text-5xl font-bold text-skynex-dark tracking-tighter leading-tight mb-6">${UI.esc(p.title)}</h1>
      <p class="text-sm text-slate-500 mb-10">
        <span class="font-semibold text-skynex-dark">${UI.esc(p.author_name)}</span>${p.author_title ? ` · ${UI.esc(p.author_title)}` : ''} · ${UI.esc(PublicAPI.formatDate(p.published_at))}
      </p>
      ${img ? `<img src="${UI.esc(img)}" alt="" decoding="async" class="w-full rounded-2xl mb-10 bg-skynex-gray">` : ''}
